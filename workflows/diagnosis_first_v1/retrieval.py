"""Diagnosis-first-v1 retrieval policy layered on shared retrieval mechanics."""
...
def step4(
    ...
    return {
        ...
        "not_assessed": [
            {
                "gene": gene,
                "reason": "no eligible diagnosis-first downstream card in this corpus version",
            }
...
def downstream(work_dir: Path) -> Path:
    ...
    if diagnosis_bundle.get("workflow_profile") != WORKFLOW_ID:
        raise ValueError(
            "diagnosis-first downstream retrieval requires diagnostic_evidence.json produced "
    ...
    if changed_tags:
        raise ValueError(
            "runtime card tags for diagnosis-first diagnosis evidence no longer match the current "
    ...
    if newly_blocked:
        raise ValueError(
            "blacklist now excludes diagnosis-first diagnosis evidence card(s): "
            + ", ".join(newly_blocked)
