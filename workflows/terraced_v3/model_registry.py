#!/usr/bin/env python3
"""Resolve terraced-v3 model-role bindings from the workflow-local model registry.
...
REGISTRY_TEMPLATE_PATH = WORKFLOW_DIR / "models.json.template"
...
def load_registry(path: Path = REGISTRY_PATH) -> dict:
    ...
    path = Path(path)
    if path == REGISTRY_PATH and not path.is_file():
