...
Terraced-v3 is an experimental NGS evidence workflow in which a **pipeline** chooses the model/provider configuration and connects diagnosis, PTBG, evidence-alignment, summarization and rendering modules.
...
- model-selected card tags are only candidates;
- fact/reason-to-card alignment is independently validated;
- sentence citations are inherited deterministically from paired facts;
- `sentence-card-interpretations.yaml` is generated deterministically from final sentence/card-tag pairs.
