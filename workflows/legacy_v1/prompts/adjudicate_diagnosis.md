...
Top-level diagnostic fields preserve the model adjudication. Set `user_review` and the initial `downstream_filter_disease` exactly as required by the current workflow mode in `workflows/legacy_v1/SKILL.md`. Do not anticipate a manual-review decision.
...
- `user_review` matches the workflow mode declared by `workflows/legacy_v1/SKILL.md`.
