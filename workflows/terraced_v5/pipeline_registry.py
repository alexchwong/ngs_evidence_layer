"""Small provider/model pipeline registry for terraced-v5."""
...
import yaml
from workflows.terraced_v5.model_binding import Binding

HERE=Path(__file__).resolve().parent
ROOT=HERE/'pipelines'
ROLES=(
    'structure','diagnosis','ptbg','statement_generation','statement_audit','evidence_match','evidence_audit',
...
def load_yaml(path:Path)->PipelinePlan:
    ...
    if meta.get('version')!=1: raise ValueError(f'unsupported terraced-v5 pipeline version {meta.get("version")!r}; expected 1')
...
def load(name:str):
    paths=_paths()
    if name not in paths: raise ValueError(f'unknown terraced-v5 pipeline {name!r}; choose one of: {", ".join(paths)}')
