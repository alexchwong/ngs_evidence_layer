...
- Within one bucket, variants sharing the same clinical proposition, framework, context, polarity, and qualifiers MUST be consolidated into one row with one shared `reason`. Do not emit parallel rows that differ only by variant identity.
