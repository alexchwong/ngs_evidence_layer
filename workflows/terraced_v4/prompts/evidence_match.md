...
For EVERY supplied evidence item, select the SINGLE candidate card that most directly supports that item's clinical reason.
...
Do not rewrite clinical reasons to fit cards. Treat any per-item previous auditor concern as useful but NON-AUTHORITATIVE feedback. The auditor can be wrong; you may re-select the same card if it remains the best semantic match.
