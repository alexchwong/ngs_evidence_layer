"""Deterministic structural validators for terraced-v4 prototype proformas.
...
def validate_evidence_match(text: str, candidate_ids: set[str]) -> str:
    issues: list[ValidationIssue] = []
    d = _doc(text, "evidence match", issues)
    _exact(d, {"card_id", "source", "quote"}, "evidence_match", issues)
    card_id = d.get("card_id")
    if "card_id" in d and card_id not in candidate_ids:
        issues.append(
            ValidationIssue(
                "evidence_match.card_id",
                f"selected card {card_id!r} was not among supplied candidate cards",
                "select one exact candidate card ID",
                repair_class="content",
                received=_preview(card_id),
                expected=str(sorted(candidate_ids)),
            )
        )
    if "source" in d: _text(d.get("source"), "evidence_match.source", issues)
    if "quote" in d: _text(d.get("quote"), "evidence_match.quote", issues)
    fail("evidence match", issues)
    return "evidence match structurally valid"


def validate_evidence_audit(text: str) -> str:
    issues: list[ValidationIssue] = []
    d = _doc(text, "evidence audit", issues)
    _exact(d, {"obvious_mismatch", "risk", "comments"}, "evidence_audit", issues)

    mismatch = d.get("obvious_mismatch")
    if "obvious_mismatch" in d and not isinstance(mismatch, bool):
        issues.append(
            ValidationIssue(
                "evidence_audit.obvious_mismatch",
                f"expected boolean; received {_type_name(mismatch)}",
                "serialize the existing true/false decision as a YAML boolean" if _boolean_serialization_only(mismatch) else "return the required true/false decision",
                repair_class="serialization" if _boolean_serialization_only(mismatch) else "content",
                received=_preview(mismatch),
                expected="true or false",
            )
        )
    if "risk" in d: _enum(d.get("risk"), {"none", "warning"}, "evidence_audit.risk", issues)

    comments = _list(d.get("comments"), "evidence_audit.comments", issues, allow_empty=True, item_hint="comment strings") if "comments" in d else []
    valid_comments: list[str] = []
    if comments is not None:
        for i, comment in enumerate(comments):
            if _text(comment, f"evidence_audit.comments[{i}]", issues):
                valid_comments.append(comment)
    if (mismatch is True or d.get("risk") == "warning") and comments is not None and not valid_comments:
        issues.append(
            ValidationIssue(
                "evidence_audit.comments",
                "mismatch/warning requires actionable feedback but comments are empty",
                "explain the specific mismatch or warning so a rematch can act on it",
                repair_class="content",
            )
        )

    fail("evidence audit", issues)
    return "evidence audit structurally valid"


def validate_reportable_sentences(text: str, elements: list[dict]) -> str:
    issues: list[ValidationIssue] = []
    d = _doc(text, "reportable sentences", issues)
    _exact(d, {"sentences"}, "reportable_sentences", issues)
    rows = _list(d.get("sentences"), "reportable_sentences.sentences", issues, allow_empty=not elements, item_hint="sentence mappings") if "sentences" in d else []
    if rows is None:
        rows = []
    if "sentences" in d and len(rows) != len(elements):
        issues.append(
            ValidationIssue(
                "reportable_sentences.sentences",
                f"expected {len(elements)} rows, received {len(rows)}",
                "return exactly one row for every supplied schema element, in the same order",
                repair_class="content",
    ...
    for i, raw_row in enumerate(rows):
        ...
        if row is None:
            continue
        _exact(row, {"schema_id", "sentence"}, path, issues)
        if i < len(elements):
            expected_id = elements[i]["schema_id"]
            if row.get("schema_id") != expected_id:
                issues.append(
                    ValidationIssue(
                        f"{path}.schema_id",
                        f"received {row.get('schema_id')!r}",
                        f"copy the supplied schema ID {expected_id!r} exactly",
                        repair_class="content",
                        received=_preview(row.get("schema_id")),
                        expected=repr(expected_id),
                    )
                )
        sentence = row.get("sentence")
        if _text(sentence, f"{path}.sentence", issues) and "\n" in sentence:
            issues.append(
                ValidationIssue(
                    f"{path}.sentence",
                    "sentence spans multiple physical lines",
                    "reserialize the same sentence on one physical line without changing its words",
                    repair_class="serialization",
                    received=_preview(sentence),
                    expected="one physical-line sentence",
                )
            )
    fail("reportable sentences", issues)
...
def validate_evidence_audit_batch(text: str, items: list[dict]) -> str:
    ...
    if len(rows) != len(items):
        issues.append(ValidationIssue(
            "audits", f"expected {len(items)} rows, received {len(rows)}",
            "return exactly one audit row for every supplied evidence item, in the same order",
            repair_class="content", received=str(len(rows)), expected=str(len(items)),
        ))
    for i, raw in enumerate(rows):
        path = f"audits[{i}]"
        ...
        if row is None:
            continue
        _exact(row, {"evidence_id", "obvious_mismatch", "risk", "comments"}, path, issues)
        if i >= len(items):
            continue
        expected_id = items[i]["evidence_id"]
        if row.get("evidence_id") != expected_id:
            issues.append(ValidationIssue(
                f"{path}.evidence_id", f"received {row.get('evidence_id')!r}",
                f"copy {expected_id!r} exactly", repair_class="content",
                received=_preview(row.get("evidence_id")), expected=repr(expected_id),
            ))
        mismatch = row.get("obvious_mismatch")
        if not isinstance(mismatch, bool):
            issues.append(ValidationIssue(
                f"{path}.obvious_mismatch", f"expected boolean; received {_type_name(mismatch)}",
                "serialize the existing true/false decision as a YAML boolean" if _boolean_serialization_only(mismatch) else "return the required true/false decision",
                repair_class="serialization" if _boolean_serialization_only(mismatch) else "content",
                received=_preview(mismatch), expected="true or false",
            ))
        ...
        if comments is not None:
            for j, comment in enumerate(comments):
                if _text(comment, f"{path}.comments[{j}]", issues):
                    valid_comments.append(comment)
        if (mismatch is True or row.get("risk") == "warning") and comments is not None and not valid_comments:
            issues.append(ValidationIssue(
                f"{path}.comments", "mismatch/warning requires actionable feedback but comments are empty",
                "explain the specific mismatch or warning so a rematch can act on it", repair_class="content",
            ))
    fail("batched evidence audit", issues)
    return "batched evidence audit structurally valid"
