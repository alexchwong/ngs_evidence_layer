"""Canonical downstream model context for terraced-v6.
...
def canonical_registry(reg: dict) -> dict:
    ...
    for vid, row in (reg or {}).items():
        if not isinstance(row, dict):
            continue
        out[vid] = {k: v for k, v in row.items() if k != "variant_id"}
    return out


def registry_context(reg: dict) -> str:
    """Render the canonical variant registry as a model-facing YAML block."""
    return _yaml({"variants": canonical_registry(reg)})
