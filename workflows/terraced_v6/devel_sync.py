#!/usr/bin/env python3
"""Sync terraced-v6 developer defaults into the root user-facing config.
...
def _validate_sources(rows: list[tuple[Path, Path]]) -> None:
    ...
    if missing:
        raise RuntimeError("missing terraced-v6 default file(s): " + ", ".join(missing))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--check", action="store_true", help="fail if root shipped defaults differ from terraced-v6")
    return parser
