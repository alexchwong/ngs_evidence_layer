...
- `diagnostic_for_other_pathology`: consider only variants that did not contribute to the primary diagnosis; use when supplied authority cards support the variant as a clinically meaningful molecular signal for a distinct pathology and do not show the variant as expected, defining, or refining in the final primary diagnosis. This classification flags possible concurrent pathology for investigation; it does not by itself establish a second neoplasm. Mere occurrence in another disease is insufficient.
