...
Rules:
- For each detected variant, first assess whether the supplied cards support it defining, refining, up-staging, or not altering the starting primary disease. Do not consider concurrent pathology yet.
- Then combine those conclusions with applicable disease-level hierarchy, morphology, cytogenetics, and other molecular findings to determine the most specific primary WHO5 diagnosis.
