...
Assess whether the NGS findings TOGETHER WITH the supplied clinical picture raise sufficient suspicion for an inherited predisposition to warrant dedicated germline evaluation. When no NGS variants are detected, return `classification: []`; do not create a pseudo-variant for a negative NGS result.
