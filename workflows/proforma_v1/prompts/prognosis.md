...
If the authoritative WHO5 result uses `schema_disease: no_haematological_malignancy` because no myeloid diagnosis is currently established, return `prognostic_frameworks: []` as well as `classification: []`. Do not assign a disease-specific prognostic framework from bootstrap retrieval categories or nonspecific clinical findings. This guard does not apply when an actual morphologic or integrated diagnosis is established: a case with zero NGS variants may still receive a framework classification such as ELN 2022 from qualifying cytogenetic or other supplied findings.
