...
- Do not emit two rows for the same variant in the same treatment category. When several therapies share one category, combine them into one row and name the therapies together in `therapy`.
- For `no_drug_implication`, omit the `therapy` field entirely; do not return an empty string or a null placeholder.
