...
The sync owns only `config/settings.json.template` and filenames present in `workflows/proforma_v1/pipelines/`. It never writes the user-owned `config/settings.json` and never deletes extra files under `config/pipelines/`.
