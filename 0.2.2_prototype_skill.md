...
Generate the diagnostic rule view exactly. The generator renders the prompt-owned diagnosis template under `prompts/workflow/prototype/`, injects the shared `prompts/workflow/reporting_rule_policy.md`, then appends only canonical R0-R1 rules:
...
First generate the rule view and branch state exactly. The generator renders the appropriate prompt-owned branch template under `prompts/workflow/prototype/`, injects the shared reporting-rule policy, and then appends either canonical R2-R5 rules or canonical R0-R5 rules when CMC changed:
