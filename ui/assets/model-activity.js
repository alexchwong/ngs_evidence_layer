...
async function poll(){if(busy)return;const selected=selectedRun();if(!selected){run='';offset=0;pending='';snapshot=null;render();return}const selectedRow=[...document.querySelectorAll('.run-row')].find(row=>String(row.getAttribute('title')||'').trim()===selected);if(!selectedRow?.querySelector('.run-dot.active'))return;if(selected!==run){run=selected;offset=0;pending='';snapshot=null;render()}busy=true;const previousOffset=offset;try{const d=await request(`/api/model-activity?run=${encodeURIComponent(run)}&offset=${offset}`);const next=Number(d.offset||0);if(next<previousOffset){offset=0;pending='';snapshot=null}consumeText(d.text||'');offset=next;render()}catch(_){if(!snapshot)panel.hidden=true}finally{busy=false}}
