#!/usr/bin/env python3
"""Unit tests for renderer card-to-reference mapping."""
import copy
...
def card(card_id, interpretation, category="diagnosis", year=2020, secondary=None,
         citation="Publication", tier="guideline criterion", publication_key="pub"):
    return {
        "card_id": card_id, "category": category, "genes": ["GENEA"],
        "diseases": ["MDS"], "evidence_tier": tier,
        "interpretation": interpretation, "locator": "fixture",
        "publication_key": publication_key, "publication_year": year,
        "citation_display": citation, "citation_incomplete": [],
...
def bundle(cards, genes=None):
    return {
        "step": 4, "genes": genes or ["GENEA"],
        "provisional_disease": "MDS", "refined_disease": "MDS",
        "diagnostic_adjudication": {
            "status": "criteria_met", "provisional_disease": "MDS",
            "refined_disease": "MDS", "downstream_filter_disease": "MDS",
            "diagnostic_label": None, "driven_by": [], "criterion_assessment": [],
            "reason": "Fixture.",
        },
        "provenance": {"corpus_version": "1.1", "corpus_sha256": "0" * 64,
                       "retrieved_at": "2026-01-01T00:00:00+00:00"},
        "retrieved": cards, "suppressed": {"count": 0, "by_disease": {}},
        "not_assessed": [],
    }


class RenderMappingTests(unittest.TestCase):
    def test_evidence_body_has_no_inline_card_ids_or_markers(self):
        cards = [card("C1-1", "Interpretation text.")]
        text = render.render(bundle(cards))["text"]
        self.assertIn("Interpretation text.", text)
        self.assertNotIn("[C1-1]", text)
    ...
    def test_one_card_primary_only(self):
        ...
        self.assertEqual(result["card_reference_map"], [{
            "card_ids": ["C1-1"], "primary_refs": [1], "secondary_refs": [],
        }])
        self.assertIn("C1-1: primary ref 1", result["text"])

    def test_one_card_primary_and_secondary(self):
        secondary = {"display": "Secondary publication", "citation_incomplete": []}
        result = render.render(bundle([card("C1-1", "Text.", secondary=secondary)]))
        self.assertEqual(result["card_reference_map"], [{
            "card_ids": ["C1-1"], "primary_refs": [1], "secondary_refs": [2],
        }])
        self.assertIn("C1-1: primary ref 1; secondary ref 2", result["text"])

    def test_cards_sharing_primary_are_grouped(self):
        cards = [
            card("C1-1", "Same text."),
            card("C1-2", "Same text."),
            card("C1-3", "Other text.", publication_key="pub-other"),
        ]
    ...
    def test_different_secondary_mappings_not_grouped(self):
        ...
        result = render.render(bundle(cards))
        self.assertIn("C1-1: primary ref 1", result["text"])
        self.assertIn("C1-2: primary ref 1; secondary ref 2", result["text"])
    ...
    def test_primary_and_secondary_roles_not_conflated(self):
        ...
        result = render.render(bundle(cards))
        self.assertIn("C1-1,C1-2: primary ref 1; secondary ref 2", result["text"])
        self.assertNotIn("C1-1: primary ref 1; secondary ref 2", result["text"])

    def test_identical_interpretations_collapse_retaining_fact_to_card(self):
        cards = [
            card("C1-1", "Identical text."),
            card("C1-2", "Identical text."),
        ]
        result = render.render(bundle(cards))
        self.assertEqual(len(result["rendered_facts"]), 1)
        self.assertEqual(result["rendered_facts"][0]["card_ids"], ["C1-1", "C1-2"])
        self.assertEqual(result["text"].count("Identical text."), 1)

    def test_collapsed_fact_may_produce_multiple_mapping_lines(self):
        secondary = {"display": "Secondary publication", "citation_incomplete": []}
        cards = [
            card("C1-1", "Identical text."),
    ...
    def test_reference_numbering_first_appearance_deterministic(self):
        ...
        result = render.render(bundle(cards))
        self.assertEqual([r["number"] for r in result["references"]], [1, 2])
    ...
    def test_missing_citation_placeholder_remains_numbered(self):
        cards = [card("C1-1", "Text.", citation=None)]
        result = render.render(bundle(cards))
        self.assertEqual(result["references"][0]["display"], "[citation missing]")
        self.assertIn("C1-1: primary ref 1", result["text"])

    def test_truncated_cards_disappear_from_all_mappings(self):
        cards = [
    ...
    def test_references_used_only_by_truncated_cards_disappear(self):
        cards = [
            card("C1-1", "Guideline text.", tier="guideline criterion", citation="Pub A"),
            card("C1-2", "Restated text.", tier="restated secondary", citation="Pub B"),
        ]
        result = render.render(bundle(cards), token_budget=100)
        self.assertEqual([r["display"] for r in result["references"]], ["Pub A"])

    def test_json_rendered_facts_and_map_match_markdown(self):
        cards = [
            card("C1-1", "Text A."),
            card("C1-2", "Text B."),
        ]
        result = render.render(bundle(cards))
        for fact in result["rendered_facts"]:
            self.assertIn(fact["interpretation"], result["text"])
    ...
    def test_every_rendered_card_occurs_exactly_once_in_map(self):
        ...
        result = render.render(bundle(cards))
        mapped = [cid for group in result["card_reference_map"] for cid in group["card_ids"]]
        self.assertEqual(sorted(mapped), ["C1-1", "C1-2", "C1-3"])
        self.assertEqual(len(mapped), len(set(mapped)))

    def test_every_mapped_number_resolves_to_bibliography_entry(self):
        secondary = {"display": "Secondary publication", "citation_incomplete": []}
        cards = [card("C1-1", "Text.", secondary=secondary)]
        result = render.render(bundle(cards))
        numbers = {n for r in result["references"] for n in [r["number"]]}
        for group in result["card_reference_map"]:
    ...
    def test_repeated_runs_byte_identical(self):
        cards = [
            card("C1-1", "Text A."),
            card("C1-2", "Text B.", secondary={"display": "Secondary", "citation_incomplete": []}),
        ]
        first = render.render(bundle(cards))
        second = render.render(bundle(copy.deepcopy(cards)))
        self.assertEqual(first["text"], second["text"])
        self.assertEqual(first["card_reference_map"], second["card_reference_map"])
