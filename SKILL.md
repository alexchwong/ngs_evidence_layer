...
`terraced-v2` supports the same five modes and is selected explicitly with `--terraced-v2`.

`terraced-v3` supports the same five modes and is selected explicitly with `--terraced-v3`. It is experimental. Its scheduler can additionally be selected with `--scheduler domain|evidence-first|variant-centric|global-ledger|adaptive-microtask`; when omitted, the v3 default is `domain`. The `--terraced` alias remains bound to v1 for compatibility.
